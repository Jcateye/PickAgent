...
PickAgent 当前承载的核心产品是 **SKU Ready Agent**：一个面向电商活动规则执行的商品准入智能体。
...
- 通过人工工作台、报告与 Agent Copilot 工作台提供查询、协同和目标驱动的长任务入口
...
- `apps/frontend/`：集中式总控制台（Next.js Dashboard + Agent Copilot）
...
- 产品主形态是双工作台：人工工作台负责确定性业务操作，Agent Copilot 工作台负责目标规划、工具编排、Trace、Context 对照和 Review Gate
- Agent runtime 分工：Pi 负责 agent harness / loop，Vercel AI SDK 负责 LLM 与 tool schema，assistant-ui 负责会话 UI
...
5. 服务端执行活动准入模拟
6. 生成 Review 清单与报告
7. 总控制台展示 Dashboard、SKU 详情、Simulation、Review、Agent Copilot
