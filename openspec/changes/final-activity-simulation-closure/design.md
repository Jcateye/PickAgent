...
`final-activity-simulation-closure` 是最终设计收敛分工中的 Layer 4B 工作包。它承接 `docs/final-design-gap-closure.md`、`docs/architecture.md`、`docs/pi-agent-copilot-design.md`、`docs/agent-backend-data-architecture.md` 和 `docs/operations/pickagent-final-design-work-allocation.md` 的边界。

## Dependencies

- Layer 0；等待 activity parse/simulation route 可用。

## Boundary

- 本 change 只覆盖 `Activities`。
...
- OpenSpec: `openspec validate final-activity-simulation-closure --strict`。
...
允许 deterministic parser fixture；默认模拟结果 SHALL 来自真实 parse/simulation API。
