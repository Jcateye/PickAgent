...
`final-review-reporting-closure` 是最终设计收敛分工中的 Layer 4B 工作包。它承接 `docs/final-design-gap-closure.md`、`docs/architecture.md`、`docs/pi-agent-copilot-design.md`、`docs/agent-backend-data-architecture.md` 和 `docs/operations/pickagent-final-design-work-allocation.md` 的边界。

## Dependencies

- Layer 0；等待 review/report route 可用，可先用 activity fixture。

## Boundary

- 本 change 只覆盖 `Reviews / Reports`。
...
- OpenSpec: `openspec validate final-review-reporting-closure --strict`。
...
允许确定性 fixture；生产默认 SHALL 使用持久化 Review/Report API snapshot。
