...
`final-cross-module-acceptance` 是最终设计收敛分工中的 Layer 4D 工作包。它承接 `docs/final-design-gap-closure.md`、`docs/architecture.md`、`docs/pi-agent-copilot-design.md`、`docs/agent-backend-data-architecture.md` 和 `docs/operations/pickagent-final-design-work-allocation.md` 的边界。

## Dependencies

- Layer 0；等待 Layer 4B、Layer 4C 全部不阻塞。复用上一轮 cross-module-integration-and-acceptance 的验收经验，但新增 final 口径。
...
- OpenSpec: `openspec validate final-cross-module-acceptance --strict`。
...
允许记录 fake fallback 的 contract 证明；最终结论 MUST 标明 L4 accepted / P0 blocker / P1 risk。
