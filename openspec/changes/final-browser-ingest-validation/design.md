...
`final-browser-ingest-validation` 是最终设计收敛分工中的 Layer 4B 工作包。它承接 `docs/final-design-gap-closure.md`、`docs/architecture.md`、`docs/pi-agent-copilot-design.md`、`docs/agent-backend-data-architecture.md` 和 `docs/operations/pickagent-final-design-work-allocation.md` 的边界。

## Dependencies

- Layer 0；等待 final-api-persistence-foundation 提供 ingest route。
...
- OpenSpec: `openspec validate final-browser-ingest-validation --strict`。
...
允许脱敏 fixture 和 mock fallback；真实提交通路必须默认走 `POST /api/ingest`。
