...
`final-agent-eventstore-foundation` 是最终设计收敛分工中的 Layer 4A 工作包。它承接 `docs/final-design-gap-closure.md`、`docs/architecture.md`、`docs/pi-agent-copilot-design.md`、`docs/agent-backend-data-architecture.md` 和 `docs/operations/pickagent-final-design-work-allocation.md` 的边界。

## Dependencies

- Layer 0；依赖 final-api-persistence-foundation 的 model / repository 决策。

## Boundary

- 本 change 只覆盖 `Agent 后端基座`。
...
- OpenSpec: `openspec validate final-agent-eventstore-foundation --strict`。
...
允许 fake adapter 写同一 EventStore contract；生产路径 SHALL NOT 让 Pi adapter 直接访问 Prisma 或业务 service。
