...
`final-copilot-overlay-closure` 是最终设计收敛分工中的 Layer 4B 工作包。它承接 `docs/final-design-gap-closure.md`、`docs/architecture.md`、`docs/pi-agent-copilot-design.md`、`docs/agent-backend-data-architecture.md` 和 `docs/operations/pickagent-final-design-work-allocation.md` 的边界。

## Dependencies

- Layer 0；等待 Agent mission/run/events route 可用。

## Boundary

- 本 change 只覆盖 `Agent Copilot 前端`。
...
- OpenSpec: `openspec validate final-copilot-overlay-closure --strict`。
...
允许 fake runtime adapter 驱动 UI；生产路径 SHALL 使用 EventStore/SSE contract。
