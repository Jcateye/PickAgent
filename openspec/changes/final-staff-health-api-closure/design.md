...
`final-staff-health-api-closure` 是最终设计收敛分工中的 Layer 4B 工作包。它承接 `docs/final-design-gap-closure.md`、`docs/architecture.md`、`docs/pi-agent-copilot-design.md`、`docs/agent-backend-data-architecture.md` 和 `docs/operations/pickagent-final-design-work-allocation.md` 的边界。

## Dependencies

- Layer 0；等待 health summary/list/detail route 可用。

## Boundary

- 本 change 只覆盖 `Dashboard / SKU Health`。
...
- OpenSpec: `openspec validate final-staff-health-api-closure --strict`。
...
允许开发 fallback；生产默认 SHALL 消费真实 API DTO。
