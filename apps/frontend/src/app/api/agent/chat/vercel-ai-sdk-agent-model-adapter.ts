...
function createPickAgentTools(input: AgentModelAdapterInput, toolExecutions: AgentConversationToolExecution[]) {
  ...
  const objectSchema = jsonSchema({
    ...
    properties: {
      ...
      tab: { type: 'string', enum: ['PENDING', 'APPROVED', 'REJECTED', 'MODIFIED', 'DRAFT'] },
      reviewType: { type: 'string', enum: ['REPLENISHMENT', 'CERTIFICATE', 'RULE_AMBIGUITY', 'ACTIVITY_CONFLICT', 'PRICE', 'AGENT_REVIEW_GATE'] },
      reviewRiskLevel: { type: 'string', enum: ['LOW', 'MEDIUM', 'HIGH'] },
      ...
      permissions: { type: 'array', items: { type: 'string', enum: ['read_product', 'read_inventory', 'write_product'] } },
      permissionKeys: { type: 'array', items: { type: 'string', enum: ['read_product', 'read_inventory', 'write_product'] } },
      allowedAgentTools: { type: 'array', items: { type: 'string' } },
      deniedRuntimeTools: { type: 'array', items: { type: 'string' } },
  ...
  return {
    ...
    listReviews: tool({
      description: '读取真实 Review 列表。支持 tab/status=PENDING/APPROVED/REJECTED/MODIFIED/DRAFT、reviewType、reviewRiskLevel=LOW/MEDIUM/HIGH、assigneeRole、dueFrom/dueTo、q/query、分页。用于用户询问待审批、高风险或某类 Review。',
      inputSchema: objectSchema,
