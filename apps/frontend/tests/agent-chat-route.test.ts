...
test('agent chat linked entities route back to the new workbench pages', () => {
  ...
  assert.equal(linkedEntityHref('dashboard', 'sku-search'), '/sku-access')
  assert.equal(linkedEntityHref('dashboard', 'sku-export'), '/sku-access')
  assert.equal(linkedEntityHref('dashboard', 'sku-ingest'), '/sku-access')
  assert.equal(linkedEntityHref('dashboard', 'activities'), '/rule-execution')
  assert.equal(linkedEntityHref('dashboard', 'dashboard'), '/overview')
  assert.equal(linkedEntityHref('dashboard', 'health-summary'), '/overview')
...
test('agent chat reads connector run and activity simulation run details', async () => {
  ...
  const connectorRetryRunId = (connectorRetry.result as { workflowRunRef?: { entityId: string } }).workflowRunRef?.entityId
  assert.ok(connectorRetryRunId)
  ...
  assert.ok(connectorRuns.linkedEntities?.some((entity) => entity.type === 'workflow_run' && entity.id === connectorRetryRunId))
