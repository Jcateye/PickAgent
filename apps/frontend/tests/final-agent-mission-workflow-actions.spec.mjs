...
test('agent mission page starts, pauses, and cancels a real mission run', async ({ page }) => {
  ...
  const pauseResponse = page.waitForResponse((response) => response.url().endsWith(`/api/agent/runs/${runId}/pause`) && response.request().method() === 'POST')
  ...
  await expect(page.getByText(`已暂停 Run：${runId}`)).toBeVisible()
  await expect(page.getByRole('link', { name: '查看暂停 Run' })).toHaveAttribute('href', `/run-console?runId=${runId}`)

  const cancelResponse = page.waitForResponse((response) => response.url().endsWith(`/api/agent/runs/${runId}/cancel`) && response.request().method() === 'POST')
  ...
  await expect(page.getByText(`已取消 Run：${runId}`)).toBeVisible()
  await expect(page.getByRole('link', { name: '查看取消 Run' })).toHaveAttribute('href', `/run-console?runId=${runId}`)
})
