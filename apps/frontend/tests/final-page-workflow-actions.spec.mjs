...
test('rule execution page runs real parsing and simulation then saves the rule set', async ({ page }) => {
  ...
  const assignReviewResponse = page.waitForResponse((response) => response.url().endsWith('/api/reviews') && response.request().method() === 'POST')
  await page.getByRole('button', { name: '批量指派' }).click()
  const assignReviewEnvelope = await (await assignReviewResponse).json()
  expect(assignReviewEnvelope.code).toBe('OK')
  expect(assignReviewEnvelope.data.length).toBeGreaterThan(0)
  expect(assignReviewEnvelope.data[0].status).toBe('OPEN')
  await expect(page.getByText(/批量指派已生成待审批 Review/)).toBeVisible()
  await expect(page.getByRole('link', { name: /查看首个 Review|查看 Review/ })).toBeVisible()

  const markReviewResponse = page.waitForResponse((response) => response.url().endsWith('/api/reviews') && response.request().method() === 'POST')
  const markDecisionResponse = page.waitForResponse((response) => /\/api\/reviews\/[^/]+\/decision$/.test(new URL(response.url()).pathname) && response.request().method() === 'POST')
  await page.getByRole('button', { name: '批量确认 Review' }).click()
  const markReviewEnvelope = await (await markReviewResponse).json()
  const markDecisionEnvelope = await (await markDecisionResponse).json()
  expect(markReviewEnvelope.code).toBe('OK')
  expect(markReviewEnvelope.data.length).toBeGreaterThan(0)
  expect(markDecisionEnvelope.code).toBe('OK')
  expect(markDecisionEnvelope.data.status).toBe('APPROVED')
  await expect(page.getByText(/批量确认已生成并批准 Review/)).toBeVisible()
