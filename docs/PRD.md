...
> **不是做一个商品健康 Dashboard，而是做一个能把活动规则转化为执行路径、检查清单、准入判断和人工 Review 的执行辅助 Agent。**
...
SKU Ready Agent 通过 Agent Copilot 工作台接收活动目标或规则文本，将其转化为结构化规则、执行路径和检查清单，再通过数据源连接器接入电商后台、平台 API、ERP 和报表数据，在服务端统一建立 SKU 健康档案。当活动规则发布时，Agent 结合商品健康数据判断哪些 SKU 可直接报名、哪些可修复后报名、哪些不建议报名，并生成修复建议、预警、报告和人工 Review 清单。
...
> **目标驱动的活动规则执行辅助 Agent，也是企业 Agent 体系里的运营执行层。**

它有两个并列入口：

```text
人工工作台：员工按页面操作，查看 Dashboard、SKU、活动模拟、Review、报告。
Agent Copilot 工作台：Agent 接收目标，规划任务，调用系统原生能力，跟踪长任务并把高风险节点交给人工 Review。
```
...
# 7.1 Agent Copilot Mission Layer

## 功能说明

Agent Copilot Mission Layer 是 Agent 工作台的核心。它不把 Agent 设计成问答窗口，而是让 Agent 成为另一个主要入口：用户给出活动目标或规则文本后，Agent 生成 Mission、规划执行路径、选择系统原生工具、启动长任务，并在不确定或高风险节点进入人工 Review。

## 输入

```text
1. 用户目标：为天猫618黄金类目准备可报名 SKU。
2. 活动规则文本：销量、库存、好评率、价格、品类配额、互斥规则。
3. 外部事件：战略 Agent、风控 Agent、活动规则变更、数据过期预警。
```

## Agent 可调用的原生能力

```text
parseActivityRules：解析活动规则为 Canonical Rule DSL
extractExecutionRequirements：提取字段、数据源、判断方式和人工确认项
checkDataFreshness：检查 SKU 快照时效性
requestConnectorIngest：请求插件或连接器采集缺失数据
diagnoseSkuHealth：运行 SKU 健康诊断
simulateActivityReadiness：执行活动准入模拟
generateExecutionChecklist：生成执行检查清单
createReviewItems：生成 Review 项
generateActivityReport：生成活动执行报告
explainDecisionWithEvidence：基于 Evidence 输出可追溯解释
```
...
```text
1. Agent Copilot 可以作为工作台入口进入。
2. 页面展示 Mission 目标、约束、自治等级和当前状态。
3. 页面展示 Agent Plan，每一步对应一个系统原生 tool。
4. 页面展示 Tool Trace、执行检查清单、长任务状态和下一步建议。
5. 页面明确显示哪些事项可自动执行，哪些事项必须人工 Review。
```

## Agent 工作台执行框架策略

Agent 工作台采用 Pi + Vercel AI SDK + assistant-ui 的分工：

```text
Pi：负责 Agent harness / loop runtime。
Vercel AI SDK：负责 LLM provider、tool schema 与 streaming 适配。
assistant-ui：负责会话 UI、消息流、输入区与工具状态展示。
业务系统：负责原子工具、Rule DSL、规则引擎、Evidence 和 Review Gate。
```

Pi 不直接访问业务数据库，不暴露默认 coding/file/bash 工具。Agent 只能调用 `AgentToolRegistry` 注册过的业务原子工具，所有高风险动作仍走 Review Gate。
...
## 8.8 Agent Copilot 工作台

### 功能说明

Agent Copilot 不是普通问答框，而是与人工工作台并列的目标驱动工作台。用户可以通过它查询 SKU 状态、解释诊断结果、规划活动规则执行路径、发起模拟、生成 Review Gate 和报告。
...
Agent 必须通过 Tool Registry 调用真实业务原子工具：
...
校验：Zod
ORM：Prisma
工作流审计：typed workflow / WorkflowRun / WorkflowStep
```
...
4. 多平台同 SKU 汇总
5. 多角色权限
6. Pi continuation run、session compaction、loop guard 和更完整的 run recovery
7. 报告导出 PDF / Excel
8. 自动创建补货任务，但必须人工确认
```
...
> 我们采用 TypeScript-first 架构，前端是 Next.js，Agent 工作台用 assistant-ui 承载会话 UI，Vercel AI SDK 负责模型接入、tool schema 和 streaming 适配，Pi 负责 Agent harness / loop runtime。插件使用 Plasmo，数据库是 PostgreSQL，ORM 是 Prisma，Zod 负责所有输入和 LLM 输出校验，WorkflowRun / WorkflowStep 负责审计和可恢复的执行记录。
